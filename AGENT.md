...
Phase 32 已完成相互作用 N=6 soft-Coulomb 的独立 \(D\) 与 \(K\) 收敛：固定 D=10 的 K=1,2,4 与固定 K=4 的 D=8,10,12 均给出变分能量非增，D12 相对同基组 CI 误差为 1.04729e-4，所有结构性及允许 materialization 的反对称残差为零。Phase 33 又在严格 value/gradient parity 下批处理 transition/factor 轴，使注册的 CPU forward/backward kernel 相对参考实现加速 34.926 倍；Blackwell 路径通过一致性和资源门限，但完整匹配求解仍比 CPU 慢 2.352 倍，因此 ADR 0022 接纳两种后端而保留 CPU 默认。唯一 active 数值任务为 Phase 34：在不读取 CI 真值的条件下实现并审计 N=6,D=12 的 K=4→5→6 自适应相关增长/可选剪枝；若不能稳定改善就记录饱和并停止，不得以扩大 N 或重启高维形式秩搜索替代该闭环。
