...
| A symmetric explicit correlator times an exterior carrier gives a low-D tradeoff beyond fixed finite NOCI | internal numerical evidence, but complete Phase 44 gate failed | Phase 40 clean N2 reproduction; Phase 44 raw samples/checkpoints and independent verifier | conservative advantage passes at interacting N4 D4/D6 and fails at D8; every blind selection run misses its frozen SE or ESS gate, so no admitted practical-method claim |
| A separate FEMPS method paper is justified | gate closed | Phase 44 overall gate failed; no external reproduction and no genuinely matched Li--Waintal/same-basis-DMRG tradeoff | retain one combined manuscript; do not draft Paper B before an external or separately preregistered matched gate passes |
