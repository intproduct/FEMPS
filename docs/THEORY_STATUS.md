...
| `T2` upper-triangular matrix-pair powers define a distinct tractable family beyond finite LC-AGP | disproved: arbitrary boundaries collapse exactly to at most `binom(M+2,2)+2` AGPs, with exact-rational M=1--6 certificate and complex128 state/gradient checks | Generalize or refute the collapse for commutative semisimple quotient and uniformly bounded radical nilpotency |
