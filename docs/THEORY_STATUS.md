...
| Symmetric explicit correlator times a `chi=1` exterior carrier separates continuous correlation control `P` from carrier basis `D` | **bounded preregistered numerical evidence with clean-source reproduction plus fixed-state VMC validation**: all 72 correlated and 54 fixed-`K` NOCI controls pass symmetry/gradient/quadrature/reconstruction checks; the frozen `P=5`/`K=4` rule passes for `D=2,4,6,8` and fails for `D=10,12`; clean rerun differences are zero. A general-small-`N` coordinate backend then passes two `N=2` deterministic-truth checks and the exact `N=4` noninteracting zero-variance/resume gate without a `D^N` tensor | The low-`D` advantage remains explicitly materialized `N=2` evidence, and Phase 43 is only fixed-state estimator validation. Interacting `N=4` optimization, systematic `D` convergence, external replication, and a Li--Waintal or same-basis DMRG comparison remain required before any method-paper claim |
