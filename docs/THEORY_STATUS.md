...
Phase 43 now supplies the first controlled `N>2` backend checkpoint without
claiming interacting physics. The continuous coordinate-space evaluator uses a
symmetric Gaussian correlator multiplying a `chi=1` exterior Slater carrier;
its analytic local energy, multi-chain sampling, uncertainty estimates,
covariance gradients, and checkpoint/resume path do not materialize a `D^N`
coefficient tensor. Two frozen `N=2` sample sets reproduce deterministic
quadrature energies within the preregistered errors, with ESS above `8.3e4`
and R-hat within `1.6e-5` of one. The canonical `N=4` noninteracting state has
energy 8, numerical variance `4.96e-31`, and exact clean/resume samples. All
sampled antisymmetry residuals are below `7.2e-16`. The chosen `N=2` state is
near stationary, so its absolute gradient pass is not strong relative-gradient
evidence. Interacting `N=4` production remains unauthorized until a separate
ADR freezes a non-NOCI differentiator and all sampling/optimization gates.

Phase 44 executes that interacting differentiator and closes negatively at the
complete preregistered level. The selected `D=4,6,8` states have held-out
combined energies `11.0243089336`, `11.0231801947`, and `11.0231265435`.
Their uncertainty-inflated error ratios against existing fixed-K4 NOCI are
`0.0320`, `0.3783`, and `0.8676`, so the physical point gate passes at
consecutive `D=(4,6)` and fails at `D=8`. All six confirmations, seed
agreements, antisymmetry checks, `D` monotonicity, and the forced D6 clean/
resume trajectory pass. The overall experiment still fails: D4 selection
standard errors exceed `2.5e-4`, while D6/D8 selection ESS values are
`47,445--49,733`, below the frozen 50,000 minimum. No thresholds or sample
counts are changed. This is internal numerical evidence, not an admitted
practical solver or publication trigger.
