...
| Statistics-carrier tensor factorization | elementary dimension proof plus exact N=3--8 ranks | symmetry-adapted TN; sign and exterior `GL(V)` representations; Grassmannian secants | a fixed carrier whose Slater multiplicity is one cannot tensor-factor every cut support: a two-Slater `N`-form has `r_1=N+2`, not divisible by the forced carrier dimension `N` | symmetry-adapted TN is invalid; no Hamiltonian-specific symmetry can help; every categorical carrier is impossible |
