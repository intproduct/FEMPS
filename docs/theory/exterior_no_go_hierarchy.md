...
flowchart TD
    ...
    E --> E0[site-indexed one-form cores]
    E --> E1[homogeneous pair-power cores]
    E --> E2[restricted coefficient memory]
    E0 --> H0[Phase 26 fixed-bond Cayley squared norm]
    E1 --> H1[Phase 13 shift-tag Cayley squared norm]
    E2 --> F1[bounded p,d]
    ...
    H0 --> S[no generic exact FEMPS solver]
    H1 --> S
    L1 --> S
    L2 --> S
    H2 --> S
...
For an `N x N` matrix `A=(A_ij)` over `Mat_2(Q)`, use the site-indexed
matrix-valued one-form cores
...
generic exact squared-norm contraction polynomial in `(N,D)` even for the
fixed class `chi<=3` would imply `FP=#P`. This is a polynomial-time Turing
reduction over exact rational input.

The earlier Phase 13 shift-tag pair-power theorem remains an independent
mechanism: it embeds the same source in a homogeneous matrix-valued pair power,
then in one-form FEMPS with maximum bond `O(N^2)`. It isolates growing
noncommutative order memory, but is no longer needed for the strongest generic
consequence.
...
fixed `Mat_2` homogeneous pair power needs at most `binom(M+3,3)` AGPs even
though the row-ordered determinant over `Mat_2` is hard. The Phase 26 direct
one-form theorem preserves row order through physical site labels; repeating
one even two-form instead symmetrizes all factor orders. For the homogeneous
pair-power family, the Phase 13 reduction still requires growing order memory.
...
one-form matrix-wedge ansatz, C4 independently proves generic joint-polynomial
hardness. C1 is strictly stronger in its bond parameter, while C4 remains the
simplest sparse-path/APG mechanism and uses only the standard 0--1 permanent.
