...
calculations support the proof audit.  A nonbranching exterior subclass is also
...
The sharp bond boundary must be stated carefully.  The Cayley amplitude state
in the proof already has maximum bond $\chi=2$ and its norm is $x^2$.  The
proved signed-amplitude recovery, however, adjoins the scalar reference
$\Theta$ and therefore uses maximum bond three.  Consequently the theorem
above proves exact squared-norm hardness for fixed $\chi\le3$; it does
\emph{not} prove the corresponding statement with the promise $\chi=2$.

\begin{conjecture}[Bond-two exact squared-norm boundary]
Exact squared-norm evaluation remains \#P-hard for rational one-form
\FEMPS{} whose maximum internal bond is two.
\label{conj:chi2-norm}
\end{conjecture}

Conjecture~\ref{conj:chi2-norm} is a precise open sharpening, not a theorem or
an exact certificate.  The established $\chi=2$ fact is the pointwise hard
amplitude encoding; the established norm theorem has maximum bond three.
...
coordinate.  We do not infer a functional-input complexity theorem from that
observation alone.  Such a theorem additionally needs a polynomial-bit model
for representing, evaluating, and interpolating the chosen functional basis.
Absent those hypotheses, that complexity transfer remains a conjectural
extension rather than a corollary of the finite rational theorem.
...
a restricted numerical control.  A nonbranching diagonal-path subclass represents a sum
of $K$ Slater paths and evaluates determinant transition quantities in
$O(K^2)$ path pairs without enumerating all virtual histories or materializing
the alternating coefficient tensor.  As numerical evidence, for four
interacting one-dimensional harmonic fermions with a finite-rank separable
interaction, blind optimizations at $(D,K)=(6,4)$ agree with finite-basis FCI
to $1.9\times10^{-12}$ in energy over three seeds, while a truth-free
$D=6\to7$ continuation has maximum finite-basis error
$8.1\times10^{-5}$ and variance $1.1\times10^{-3}$.  At $D=6$, the error falls
from $5.3\times10^{-3}$ at $K=1$ to $2.3\times10^{-11}$ at $K=2$; at $K=4$,
the continuum error decreases monotonically over $D=5,6,7$.  Norm errors are
below $5\times10^{-16}$ and structural and materialized antisymmetry residuals
are zero.  Two additional clean-source schedules select different determinant
paths but give a combined final-energy spread of $2.035\times10^{-9}$, maximum
same-basis CI error $2.523\times10^{-9}$, maximum variance
$1.462\times10^{-8}$, and no optimization failures.  These data are retained
in this manuscript as a bounded numerical exercise.  The subclass is exactly
a finite nonorthogonal Slater sum, so the calculation does not establish an
ansatz beyond NOCI, an explicit-correlation advantage, improved functional-
basis $D$ convergence, asymptotic scalability, or superiority over FCI,
particle TT, Li--Waintal, or same-basis DMRG.
...
\midrule
Fixed-bond norm & $2\le N\le6$ & Direct Cayley coefficient, four boundaries, explicit bond-three sum, norm polarization, exact antisymmetry. \\
...
the Veronese powers
$(t_1z_1+\cdots+t_vz_v)^M$ span
$\operatorname{Sym}^M(\mathbb C^v)$.  One direct proof differentiates
$(t\cdot z)^M$ at $t=0$: every monomial $z^\alpha$, $|\alpha|=M$, is a linear
combination of evaluations at finitely many rational points by multivariate
finite differences.  Equivalently, choose
$\binom{M+v-1}{v-1}$ generic rational points; the resulting Veronese
evaluation matrix is nonsingular because its determinant is a nonzero
polynomial.  Therefore any homogeneous degree-$M$ polynomial in $v$ variables
is a linear combination of at most
$\binom{M+v-1}{v-1}$ $M$th powers.
