...
calculations support the proof audit.  These exact obstructions constrain, but
do not exclude, restricted polynomially contractible subclasses or controlled
approximate and stochastic algorithms; they do not establish a scalable
generic fermionic solver.
\end{abstract}
...
\begin{theorem}[Exact TT ranks and exchange floor]
...
\begin{proposition}[Known flat Slater particle spectrum]
...
Theorem~\ref{thm:r1} and Proposition~\ref{thm:r2} concern labelled-particle TT only; they do
...
a restricted solver.  A nonbranching diagonal-path subclass represents a sum
...
are zero.  These small-system data demonstrate a viable restricted route, not
generic polynomial contraction, asymptotic scalability, or superiority over
FCI, particle TT, or DMRG.
...
method should be evaluated as separate publication projects.  Higher-
dimensional alternating-form rank classification is not pursued here unless a
specific rank question determines a contraction rule, expressivity bound,
truncation criterion, or physical benchmark.  The current algorithmic program
instead tests restricted exact contraction and error-controlled approximate
routes on continuous interacting models.
...
contains a permanent.  The resulting map is an algorithm-design constraint,
not a denial of numerical \FEMPS{}: polynomially contractible restricted
subclasses and error-controlled approximate methods remain open and must be
judged by interacting benchmarks, convergence, cost, uncertainty, and
antisymmetry residuals rather than by parameter counts alone.
